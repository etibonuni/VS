...
def doMolIsoF(molNdx):
    ...
    for portion in datasetPortion:
                ...
                for fold in range(0, folds):
                    try:
                        ...
                        for i in range(0, folds):
                            if i != fold:
                                if train_ds is None:
                                    train_ds = [r[0].iloc[0:1, :] for r in folds_list[i]] # Take only LEC
                                else:
        ...
        auc = eval.plotSimROC(results["truth"], [results["score"]],
                              ...
                              "results/"+molName + "_IsoForest_sim_" + str(portion * 100) + ".pdf")
        auc_rank = eval.plotRankROC(results["truth"], [results["score"]],
                                    molName + "[IsoForest, " + str(portion * 100) + "%]",
                                    "results/"+molName + "_IsoForest_rank_" + str(portion * 100) + ".pdf")
...
p.map(doMolIsoF, range(0, len(molfiles)));